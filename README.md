...
- 1.x 19 项测试与 2.0 59 项测试全部通过，合计 78 项。
...
当前仓库版本为 **2.0.0-alpha.6**。面向用户的稳定本地功能仍以 **1.2.0** 为基线；Alpha 新增用户/家庭/RBAC、同步协议、显式迁移、服务端接口、运行时请求校验与限流，以及 PostgreSQL 身份/会话、家庭/成员/邀请、8 类同步命令、写事务执行器与一致性读模型，但尚未完成隐私/迁移生产组合和真实实例验证。
