...
- 1.x 19 项、TypeScript 2.0 75 项与 Django/SQLite 后台 26 项测试全部通过，合计 120 项。
...
当前仓库版本为 **2.0.0-alpha.12**。面向用户的稳定本地功能仍以 **1.2.0** 为基线；Alpha 已新增可真实迁移的 uv + Django 5.2 + SQLite 后台，覆盖身份、家庭、同步、完整 v1 迁移、导出、注销、限流和一致性备份。部署按用户要求等待最终域名与服务器方案；小程序云模式和双设备联调尚未完成。
