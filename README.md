...
- 1.x 19 项测试与 2.0 48 项测试全部通过，合计 67 项。
...
当前仓库版本为 **2.0.0-alpha.3**。面向用户的稳定本地功能仍以 **1.2.0** 为基线；Alpha 新增用户/家庭/RBAC、同步协议、显式迁移、服务端接口、运行时请求校验与限流、PostgreSQL 写事务执行器与一致性读模型、家庭管理和数据权利页面，但尚未完成生产 PostgreSQL 服务接线与真实实例验证。
