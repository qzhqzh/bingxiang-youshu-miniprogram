...
| 数据与账号 | 脱敏导出云端数据、申请账号注销、查看冷静期并取消申请 |
...
- 公开 API 已有严格运行时 schema、可替换限流器、数据导出和注销冷静期契约；敏感限流键只保存哈希。
...
- 1.x 19 项、TypeScript 2.0 76 项与 Django/SQLite 后台 26 项测试全部通过，合计 121 项。
- 小程序与服务端 TypeScript 严格检查 0 错误。
- 14 个页面与 137 个小程序文件通过静态检查。
- JSON、WXML、本地资源和 Repository 边界检查通过。
- 当前主包约 467.5 KiB。
- 登录/网络实现只存在于 Remote Gateway，发布配置关闭云同步且 API 域名为空。
- 1.2.0 已完成微信开发者工具实际渲染；2.0 的同步状态、冲突中心、家庭空间、家庭成员和数据与账号页也已完成开发者工具专项渲染回归。

完整记录见 [TEST_RESULTS.md](./TEST_RESULTS.md)。

### 2.0 API 本地/预发启动

服务端配置只从环境变量读取，不要把 AppSecret 或数据库连接串写入仓库。准备独立 PostgreSQL 后：
...
当前仓库版本为 **2.0.0-alpha.13**。面向用户的稳定本地功能仍以 **1.2.0** 为基线；Alpha 已新增 uv + Django 5.2 + SQLite 后台，并让首页、仓库、食谱、做菜、购物清单和我的页面在登录后读取家庭云数据、通过离线命令队列写入。部署按用户要求等待最终域名与服务器方案；真实微信身份、HTTPS 域名和双设备联调仍待完成。
