...
- 1.x 19 项测试与 2.0 55 项测试全部通过，合计 74 项。
...
当前仓库版本为 **2.0.0-alpha.5**。面向用户的稳定本地功能仍以 **1.2.0** 为基线；Alpha 新增用户/家庭/RBAC、同步协议、显式迁移、服务端接口、运行时请求校验与限流、PostgreSQL 身份/会话、家庭/成员/邀请事务服务、写事务执行器与一致性读模型、家庭管理和数据权利页面，但尚未完成全部 PostgreSQL 命令接线与真实实例验证。
