...
- 1.x 19 项测试与 2.0 63 项测试全部通过，合计 82 项。
...
当前仓库版本为 **2.0.0-alpha.7**。面向用户的稳定本地功能仍以 **1.2.0** 为基线；Alpha 新增用户/家庭/RBAC、同步协议、显式迁移、服务端接口、运行时请求校验与限流，以及 PostgreSQL 身份/会话、家庭协作、8 类同步命令、一致性读模型和数据导出/注销 worker，但尚未完成迁移服务生产组合和真实实例验证。
