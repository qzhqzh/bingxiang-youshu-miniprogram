...
更新时间：2026-08-13
当前阶段：`2.0.0-alpha.3`，阶段 0/1 与部分阶段 3 的可运行工程骨架；**不是可连接真实用户数据的生产版本**。
...
- PostgreSQL schema、mutation 事务执行器和一致性 Query Store 已存在，SQL 边界已通过模拟连接测试；尚未实现完整 `PostgresV2Service` 命令/账号服务并接入 API，也未在真实 PostgreSQL 实例执行集成测试。
...
1. 完整 PostgreSQL 命令/账号服务、API 运行时接线与真实数据库集成测试；Query Store 和事务执行器已有只读快照、`SELECT … FOR UPDATE`、cursor 和幂等提交边界。
...
- 1.x：19 项领域与闭环测试通过。
- 2.0：48 项身份、RBAC、租户隔离、同步、并发库存、迁移、HTTP schema/限流、家庭切换、数据权利、PostgreSQL 读/写事务边界和冲突处理测试通过。
- 合计：67 项测试通过。
