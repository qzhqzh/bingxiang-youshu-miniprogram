...
| 数据删除不可恢复或删错租户 | 注销冷静期、owner 前置校验、受限会话、`SKIP LOCKED` worker、匿名共享事实、备份保留和恢复演练 | 内存状态机与 PostgreSQL SQL 边界测试已完成；真实数据库删除/恢复演练待实现 |

## 上线前仍需关闭的高风险缺口

- PostgreSQL 身份、家庭协作、读模型、v1 数据迁移、mutation 事务边界和生产 API 组合已实现；数据库 migration、约束、断线恢复和并发锁仍未在真实实例验证。
- API 已接入运行时 schema 与单进程可替换限流器；Redis 多副本限流、指标、告警和集中脱敏日志尚未实现。
- 用户导出、注销冷静期、删除领域任务和独立生产 worker 已实现；加密导出存储、过期物理清理和真实备份删除边界尚未验证。
